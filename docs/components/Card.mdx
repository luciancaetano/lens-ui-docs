import { Badge, LensProvider, IntentEnum, Card, Layout } from 'lens-ui';
  ...
  <LensProvider>
    <Layout.Content layout="horizontal" style={{padding: "1rem", flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center'}}>
      {Object.keys(IntentEnum).map(((intent) => (
        <Card.Card style={{marginRight: '1rem', marginBottom: '1rem', maxWidth: '250px'}} intent={intent}>
          <Card.Body>
            <Card.Title>{intent.toUpperCase()}</Card.Title>
            <Card.Title subtitle>subtitle</Card.Title>
          </Card.Body>
          <Card.Image src={`https://picsum.photos/480?${intent}`} />
          <Card.Body>
            <Card.Text>
